...
export const captureStripePaymentService = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const createStripeCheckoutSessionService = async (
  ...
  const lineItems = (req as AuthenticatedRequestBody<ProcessingStripeCheckoutT>).body.orderItems.map((item) => {
    return {
      price_data: {
        currency: 'usd',
        product_data: {
